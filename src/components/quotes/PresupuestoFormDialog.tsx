...
export function PresupuestoFormDialog({ open, onOpenChange, presupuesto }: Props) {
  ...
  const { data: clientes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('clientes').select('id, nombre, ruc, sucursal').eq('activo', true).order('nombre');
      return data || [];
    },
  });
  ...
  const { data: productos = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('productos').select('*').eq('activo', true).order('descripcion');
      return data || [];
    },
  });
  ...
  useEffect(() => {
    if (presupuesto && open) {
      ...
      setValidoHasta(presupuesto.valido_hasta || '');
      setCondicion(presupuesto.condicion || 'contado');
      setMoneda(presupuesto.moneda || 'PYG');
      setFxRate(Number(presupuesto.fx_rate) || 1);
      setObservacion(presupuesto.observacion || '');
      supabase.from('presupuesto_items' as any).select('*').eq('presupuesto_id', presupuesto.id)
        .then(({ data }) => {
          if (data?.length) {
  ...
  const selectProducto = (i: number, productoId: string) => {
    ...
    next[i] = {
      ...
      precio_unitario: Number(p.precio), iva: p.iva,
    };
    setItems(next);
  };
  ...
  const handleSave = async () => {
    if (!clienteId || items.length === 0) { toast.error('Selecciona cliente y al menos un ítem'); return; }
    ...
    setSaving(true);
    try {
      const autoNumero = numero || `PRES-${Date.now().toString().slice(-7)}`;
      const payload: any = {
        empresa_id, cliente_id: clienteId, numero: autoNumero,
        fecha, valido_hasta: validoHasta || null,
        condicion, moneda, fx_rate: fxRate,
      ...
      let presId: string;
      if (presupuesto) {
        const { error } = await supabase.from('presupuestos' as any).update(payload).eq('id', presupuesto.id);
        if (error) throw error;
        presId = presupuesto.id;
        await supabase.from('presupuesto_items' as any).delete().eq('presupuesto_id', presId);
      } else {
        const { data, error } = await supabase.from('presupuestos' as any)
          .insert({ ...payload, estado: 'borrador', created_by: user?.id })
          .select('id').single();
        if (error) throw error;
        presId = (data as any).id;
      }
      ...
      const { error: itemsErr } = await supabase.from('presupuesto_items' as any).insert(itemsData);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{presupuesto ? 'Editar presupuesto' : 'Nuevo presupuesto'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          {moneda !== 'PYG' && (
            <div className="space-y-2">
              ...
              <Input type="number" min={0} step="0.01" value={fxRate} onChange={(e) => setFxRate(Number(e.target.value))} />
            </div>
          )}
        </div>
              ...
              <TableBody>
                {items.map((it, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <Input type="number" min={1} step="1" value={it.cantidad} onChange={(e) => updateItem(i, 'cantidad', Number(e.target.value) || 1)} />
                    </TableCell>
                    <TableCell>
                      <Input type="number" min={0} step="1" value={it.precio_unitario} onChange={(e) => updateItem(i, 'precio_unitario', Number(e.target.value))} />
                    </TableCell>
                    <TableCell>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving}>
            {saving ? 'Guardando...' : presupuesto ? 'Actualizar' : 'Crear presupuesto'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
